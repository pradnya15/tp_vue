  ...
  <form class="form-signin"  @submit.prevent="submit">
    <!--<form class="form-signin" @submit.prevent="login">-->
      ...
      <div class="form-group has-float-label">
        <input class="form-control" v-bind:class="{'has_error': login_error }" id="email" type="email" required placeholder="username@companydomain.com"/>
        <label for="email">Email</label>
        <div class="error_msg" v-if="login_error">Please enter a valid email.</div>
      </div>
      <div class="form-group has-float-label">
        <input class="form-control" id="password" :type="passwordFieldType" required v-model="password" placeholder="••••••••" style="padding: 22px 50px 7px 10px;"/>
...
import forgotPWD from '../components/forgotPassword.vue'
export default {
  name: 'Login',
  data () {
    return {
